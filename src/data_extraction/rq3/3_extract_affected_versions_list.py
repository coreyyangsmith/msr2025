...
def process_cve(row):
    ...
    try:
        ...
        processed_result = {
            ...
            "cve_patched": result["cve_patched"],
            "affected_versions": ",".join(result["affected_versions"])
        ...
        pd.DataFrame([processed_result]).to_csv(
            "data/rq3_3_affected_versions.csv",
            mode="a",
            header=not os.path.exists("data/rq3_3_affected_versions.csv"),
            index=False,
        )
...
if os.path.exists("data/rq3_3_affected_versions.csv"):
    os.remove("data/rq3_3_affected_versions.csv")
...
print(f"Results saved to data/rq3_3_affected_versions.csv")
