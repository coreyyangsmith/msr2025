...
def fetch_data(base_url, repo_name, type_):
    ...
    # Replace spaces and special characters in the URL if necessary
...
def save_to_csv(keys, values, acc_values, repo_name):
    filename = f"data/rq2_opendigger/{repo_name.replace('/', '_')}_data.csv"
    try:
        ...
        with open(file_path, "r") as csvfile:
            ...
            for idx, row in enumerate(rows):
                ...
                if data:
                    ...
                    save_to_csv(keys, values, acc_values, repo_name)
                else:
