...
from ...utils.config import RQ2_8_INPUT, RQ2_8_OUTPUT, OPENDIGGER_VALUES

# Read the combined dataset from step 7
df = pd.read_csv(RQ2_8_INPUT)
...
for idx, row in tqdm(df.iterrows(), total=len(df), desc="Enriching metrics data"):
    ...
    # Use patched_version_date if cve_patched is True, otherwise use current date
    target_date = (
        row["patched_version_date"]
        if row["cve_patched"]
    ...
    for metric_file in metric_files:
        metrics_path = f"data/rq2_opendigger/{repo_name}/{metric_file}"
...
df.to_csv(RQ2_8_OUTPUT, index=False)
print(f"Enriched dataset saved to {RQ2_8_OUTPUT}")
