...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["start_date"] = pd.to_datetime(df["start_version_timestamp"], errors="coerce")
    df["end_date"] = pd.to_datetime(df["end_version_timestamp"], errors="coerce")
    ...
    # df["duration_seconds"] = df["cve_duration"].apply(safe_parse)
    df["duration_seconds"] = df["cve_duration"] / 1000
...
def main():
    ...
    df = df[
        ["start_version_timestamp", "end_version_timestamp", "cve_duration", "severity"]
    ].rename(
        columns={
            "start_version_timestamp": "start_version_timestamp",
            "end_version_timestamp": "end_version_timestamp",
